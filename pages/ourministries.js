...
import { getMinistries } from '../utilities/directus';

export default function OurPurpose({ ministries }) {
  return (
    <>
      <Head>
        <title>
          Our Ministries | Schomburg Road Baptist Church Columbus Georgia
        </title>
        <meta
          name="description"
          content="A list of ministries at Schomburg Road Baptist Church"
        />
        <meta name="keywords" content="ministries" />
      </Head>

      <h1 className="container mx-auto mb-8 mt-4 border-b-2 px-8 font-headings text-4xl font-black text-secondary md:px-20">
      ...
      {ministries.map((ministry, index) => (
        <SplitScreen
          ...
          `}
          key={ministry.id}
          reverse={index % 2} // alternate the reverse prop
        />
      ))}
    </>
  );
...
export const getStaticProps = async () => {
  ...
  return {
    props: {
      ministries: ministries.data.ministries,
    },
    revalidate: 60,
  };
};
